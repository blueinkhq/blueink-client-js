  ...
  try {
    ...
    switch (action) {
      case 'Create Person': {
        ...
        const allCurrentEmails = ph.getEmails()
        ...
        const allCurrentPhones = ph.getPhones()
        ...
        const createResp = await client.persons.createFromPersonHelper(ph)
        const person = createResp.data
        console.log(`Created person ${person.id}`)
        break
      }

      case 'Update Person': {
        const person_id = await askPersonId() /* eslint-disable-line camelcase */
        const updateResp = await client.persons.update(
          person_id, /* eslint-disable-line camelcase */
          personSampleUpdate
        )
        const person = updateResp.data
      ...
      case 'Retrieve Person': {
        const person_id = await askPersonId() /* eslint-disable-line camelcase */
        const retrieveResp = await client.persons.retrieve(person_id) /* eslint-disable-line camelcase */
        const person = retrieveResp.data
      ...
      case 'Delete Person': {
        const person_id = await askPersonId() /* eslint-disable-line camelcase */
        await client.persons.delete(person_id) /* eslint-disable-line camelcase */
        console.log(`Deleted person ${person_id}`) /* eslint-disable-line camelcase */
        break
      }
    }
  } catch (error) {
...
const askPersonId = async () => {
  console.log('\nEnter a Person ID')
  let person_id = '' /* eslint-disable-line camelcase */
  while (!person_id) { /* eslint-disable-line camelcase */
    const answer = await inquirer.prompt({
      name: 'person_id',
      type: 'input'
    })
    person_id = answer.person_id /* eslint-disable-line camelcase */
  }
  return person_id /* eslint-disable-line camelcase */
}
