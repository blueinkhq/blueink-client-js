const BundleSubClient = (path, request) => {
    return {
        /**
         * Create new Bundle.
         * @param {object} data
         ...
         * @param {string} bundleId - The ID that uniquely identifies the Bundle.
         */
        cancel: (bundleId) => request.put(`${path}/${bundleId}/cancel/`),
         ...
         */
        listEvents: (bundleId) =>
            request.get(`${path}/${bundleId}/events/`),
         ...
         */
        listFiles: (bundleId) =>
            request.get(`${path}/${bundleId}/files/`),

        /**
         * Get data entered into fields for a completed Bundle.
         * @param {string} bundleId - The ID that uniquely identifies the Bundle.
         */
        listData: (bundleId) => request.get(`${path}/${bundleId}/data/`),
...
getRelatedData = async (request, path, bundle) => {
    try {
        const related_data = {};
        related_data.events = await request.get(
            `${path}/${bundle.id}/events/`
        );
        if (bundle.status === "co") {
            related_data.files = await request.get(
                `${path}/${bundle.id}/files/`
            );
            related_data.data = await request.get(
                `${path}/${bundle.id}/data/`
            );
        }
        return related_data;
