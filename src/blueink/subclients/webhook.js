...
const WebhookSubClient = (request) => {
  return {
     ...
     * @param {object} data
     */
    update: (webhookId, data) => request.put(WEBHOOKS.UPDATE(webhookId), data),
     ...
     */
    retrieveHeader: (webhookExtraHeaderId) =>
      request.get(WEBHOOKS.RETRIEVE_HEADER(webhookExtraHeaderId)),
     ...
     * @param {object} data
     */
    updateHeader: (webhookExtraHeaderId, data) =>
      request.put(WEBHOOKS.UPDATE_HEADER(webhookExtraHeaderId), data),
     ...
     * @param {object} data
     */
    partialUpdateHeader: (webhookExtraHeaderId, data) =>
      request.patch(WEBHOOKS.UPDATE_HEADER(webhookExtraHeaderId), data),

    /**
     * Delete a Webhook Extra Header.
     * @param {string} webhookExtraHeaderId - The ID that uniquely identifies the Webhook Extra Header.
     */
    deleteHeader: (webhookExtraHeaderId) =>
      request.delete(WEBHOOKS.DELETE_HEADER(webhookExtraHeaderId)),
     ...
     */
    retrieveEvent: (webhookEventId) =>
      request.get(WEBHOOKS.RETRIEVE_EVENT(webhookEventId)),
    ...
    listDeliveries: (params = {}) =>
      request.get(WEBHOOKS.LIST_DELIVERIES, params),
     ...
     */
    retrieveDelivery: (webhookDeliveryId) =>
      request.get(WEBHOOKS.RETRIEVE_DELIVERY(webhookDeliveryId)),
