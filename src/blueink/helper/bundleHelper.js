...
class BundleHelper {
  ...
  #addDocument = (newDoc) => {
    ...
    if (newDoc.file_path) {
      ...
      const file = new File(
        [newDoc.file_data.buffer],
        newDoc.file_data.originalname
      )
      this.files[`files[${newDoc.file_index}]`] = file
      ...
      const file = new File(
        [Buffer.from(newDoc.file_b64, 'base64')],
        newDoc.file_name
      )
      this.files[`files[${newDoc.file_index}]`] = file
  ...
  addDocumentTemplate = (template) => {
    ...
    if (has(template, 'assignments') && !Array.isArray(template.assignments)) {
      ...
      for (const i in template.assignments) {
    ...
    if (template.field_values && !Array.isArray(template.field_values)) {
      ...
      for (const i in template.field_values) {
  ...
  initializeField = (docKey, fieldKey, value) => {
    ...
    if (isTemplate) {
      const fieldValues = get(document, 'field_values', [])
      const index = fieldValues.findIndex((field) => field.key === fieldKey)
      if (index !== -1) {
        fieldValues[index].initial_value = value
      } else {
        fieldValues.push({
          key: fieldKey,
          initial_value: value
      ...
      document.field_values = [...fieldValues]
      return fieldKey
    } else {
      ...
      if (index !== -1) {
        ...
        throw new Error(
          `Field with key ${fieldKey} cannot be found. The field must be added first.`
        )
      }
      return fieldKey
    }
